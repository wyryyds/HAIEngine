...
namespace HAIEngine
{
	MeshFilter::MeshFilter(const std::vector<Vertex>& vertices, const std::vector<unsigned int>& indices, const std::vector<TTexture>& textures)
	...
	void MeshFilter::Draw(std::shared_ptr<OpenGLShader> shader)
	{
	...
	void Model::Draw(std::shared_ptr<OpenGLShader> shader)
	{
	...
	MeshFilter Model::processMesh(aiMesh* mesh, const aiScene* scene)
	{
		...
		return MeshFilter(vertices, indices, textures);
	}
